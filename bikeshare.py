...
def time_stats(df):
    ...
    time_data = pd.Series(data=[popular_month, popular_weekday,popular_start_hour], index=['Most common month:', 'Most common day_of_week:', 'Most common hour:'])
...
def station_stats(df):
    ...
    travel_data = pd.Series(data=[popular_start_station, popular_end_station, popular_trip], index=['Most commonly used start station:','Most commonly used end station:','Most frequent trip:']) 
    print(travel_data)                        
...
def trip_duration_stats(df):
    ...
    travel_stats = pd.Series(data=[total_travel_time, mean_travel_time], index=['Total travel time in 2017:', 'Average travel time per trip:']) 
    print(travel_stats)
...
def user_stats(df):
    ...
    if 'Birth Year' in df.columns:
        ...
        age_data = pd.Series(data = [earliest_birth_year, recent_birth_year, common_birth_year], index=['Olderst birth year:', 'Youngest birth year:', 'Common birth year:'])
        print(age_data)
    else:
